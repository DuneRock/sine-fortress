...
CEnginePatch g_EnginePatches[] =
{
    ...
    // Search "R_DrawSkybox" in ghidra
    {
        "SkyboxCulling",
        "\xf3\x0f\x59\x0d\x0a\x6e\x38\x00\xf3\x0f\x58\xc4\xf3\x0f\x58\xc1\x41\x0f\x2f\xc0",
        "xxxx????xxxxxxxxxxxx",
        20,
        PATCH_IMMEDIATE,
    ...
    {
        "SkyboxCulling",
        "\xf3\x0f\x58\xca\x0f\x28\xc3\xf3\x0f\x58\xc1\x0f\x2f\xe0",
        "xxxxxxxxxxxxxx",
        14,
        PATCH_IMMEDIATE,
        "\x90\x90\x90\x90\x90\x90"
...
void CEnginePatch::ApplyPatch()
{
    if (!m_pPatch)
    {
        Error("Engine patch \"%s\" FAILED: No value provided\n", m_sName);
        return;
    }
...
CEnginePatch::CEnginePatch(const char* name, const char* signature, const char* mask, size_t offset, bool immediate)
{
    m_sName = name;
...
CEnginePatch::CEnginePatch(const char* name, const char* signature, const char* mask, size_t offset, bool immediate, int value)
...
CEnginePatch::CEnginePatch(const char* name, const char* signature, const char* mask, size_t offset, bool immediate, float value)
    : CEnginePatch(name, signature, mask, offset, immediate)
{
    m_iLength = sizeof(value);
    m_pPatch = new char[m_iLength];
...
CEnginePatch::CEnginePatch(const char* name, const char* signature, const char* mask, size_t offset, bool immediate, char* bytes)
    : CEnginePatch(name, signature, mask, offset, immediate)
{
    m_iLength = strlen(bytes);
    m_pPatch = bytes;
}
